<table width="100%">
    <tr>
        <td nowrap="" width="40%">1. Nama Kegiatan</td>
        <td width="60%">: <b>Diisi Manual</b></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">2. Nama Organisasi Pelaksana</td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">3. Ketua Pelaksana</td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">&nbsp &nbsp &nbsp &nbsp NIM</td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">&nbsp &nbsp &nbsp &nbsp Fakultas / Prodi </td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">&nbsp &nbsp &nbsp &nbsp No. Telp / HP </td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">4. Sekertaris Pelaksana</td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">&nbsp &nbsp &nbsp &nbsp NIM</td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">&nbsp &nbsp &nbsp &nbsp Fakultas / Prodi </td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">&nbsp &nbsp &nbsp &nbsp No. Telp / HP </td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">5. Jumlah Anggota</td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">6. Pembina Organisasi</td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">&nbsp &nbsp &nbsp &nbsp Nama Lengkap</td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">&nbsp &nbsp &nbsp &nbsp No. Telp / HP </td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">&nbsp &nbsp &nbsp &nbsp Tempat Kegiatan </td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
    <tr>
        <td nowrap="" width="40%">7. Jumlah Total</td>
        <td>: <i>Diisi Manual</i></td>
    </tr>
</table>